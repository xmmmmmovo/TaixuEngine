...
public:
    ...
    virtual bool bindUniformBuffer(TXBuffer* buffer, uint32_t offset, uint32_t size) = 0;

    virtual bool draw(uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance) = 0;
    virtual bool drawIndexed(uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex, int32_t vertexOffset,
