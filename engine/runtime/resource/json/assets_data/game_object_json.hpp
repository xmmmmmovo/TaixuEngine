...
public:
    ...
    void serialize()
    {
        if (GO_path != "INVALID")
        {
            std::ofstream o(asset_file_path/GO_path);
            ...
            json write{{"TransformComponent",t},{"MeshComponent",m},{"RigidBodyComponent",r}};
    ...
    void deserialize()
    {
        std::ifstream f(asset_file_path/GO_path);
        if (!f.is_open()) {
        ...
        TransformComponent.from_json(components["TransformComponent"], TransformComponent);
        MeshComponent.from_json(components["MeshComponent"], MeshComponent);
        RigidBodyComponent.from_json(components["RigidBodyComponent"], RigidBodyComponent);
    }
};
