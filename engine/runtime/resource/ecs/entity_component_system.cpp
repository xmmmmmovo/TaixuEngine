...
void ECS::initialize() {
    ...
    auto  mesh       = std::make_shared<MeshComponent>(
            true, current_GO.get_id(), "assets/model/sphere.obj", "material");
    ...
    auto rigid_body = std::make_shared<RigidBodyComponent>(
            current_GO.get_id(), _pointer, scene_target);
    ...
    auto sphere_mesh = std::make_shared<MeshComponent>(
            true, current_GO.get_id(), "assets/model/cube.obj", "material");
...
void ECS::dataRedirection(IGraphicsContext *render_context) {
